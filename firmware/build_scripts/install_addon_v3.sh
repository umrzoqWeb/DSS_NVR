...
# ============================================================================
# === DSS NVR cloud blokirovkasi v3 (mtd6 reflash, 2026-05-17) ===
# v2 dan farq: /etc/hosts ni EDVR ISHGA TUSHGANDAN KEYIN yozamiz
# Sabab: edvr birinchi ishga tushganda /etc/hosts ga real IP'lar yozadi,
#        keyin tegmaydi. Biz uning ustidan yozsak — bizniki qoladi.
# ============================================================================

# --- 1. p2p_telnet.sh backdoor neytrallash (oldindan) ---
...
(
  ...
  # === 2b. IP route reject (34 ta cloud IP) ===
  ...
  sleep 15  # Edvr to'liq settle qilishi uchun

  # === 2d. /etc/hosts ni qaytadan yozish (edvr-ning real IP'larini qoplab) ===
  rm -f /etc/hosts
...
HOSTS_EOF
  ...
  # === 2e. Guard loop — agar kim qaytaroq yozsa, tiklash ===
  ( while sleep 60; do
      if ! grep -q "^0.0.0.0.*auth.p6sai.com" /etc/hosts 2>/dev/null; then
        rm -f /etc/hosts
...
# === DSS NVR cloud blokirovkasi v3 yakuni ===
